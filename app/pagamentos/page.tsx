...
export default function PagamentosPage() {
  ...
  async function markAsPaid(payment: Payment) {
    ...
    await supabase.from('payments').update({ status: 'pago', paid_date: today }).eq('id', payment.id)
    loadAll()
  }

  async function markAsPending(payment: Payment) {
    await supabase.from('payments').update({ status: 'pendente', paid_date: null }).eq('id', payment.id)
    loadAll()
  }
  ...
  function gerarCobrancasMes() {
    ...
    setForm({ contract_id: '', due_date: `${ano}-${mes}-10`, amount: '', notes: '' })
    setShowForm(true)
  }
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-500 mt-0.5">
                {payments.filter(p => p.status === 'atrasado').map(p => p.contracts?.tenants?.name).join(', ')}
              </p>
            </div>
          </div>
        )}

        {/* Formulário — sempre renderizado, visibilidade controlada por classe */}
        <div className={`bg-white border border-blue-200 rounded-xl p-6 mb-6 max-w-2xl transition-all ${showForm ? 'block' : 'hidden'}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="font-semibold text-gray-900">{p.contracts?.tenants?.name}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${statusStyle[p.status]}`}>
                        {p.status}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500">{p.contracts?.properties?.name}</p>
